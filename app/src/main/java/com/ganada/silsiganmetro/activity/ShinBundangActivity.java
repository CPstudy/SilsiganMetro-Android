...
public class ShinBundangActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
			...
			window.setStatusBarColor(ContextCompat.getColor(getBaseContext(), tm.getTitleBarColor(iTheme, LineManager.LINE_SHINBUNDANG)));
		}
		layTitle.setText("신분당선");
		layTitle.setBackgroundColorById(tm.getTitleBarColor(iTheme, LineManager.LINE_SHINBUNDANG));
		 ...
		 */
		listnorm = new ListNorm("강남", 1077000687, 2);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("양재", 1077000688, 1);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("양재시민의숲(매헌)", 1077000689, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("청계산입구", 1077006810, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("\t—", 1077000000, 4);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("판교", 1077006811, 1);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("정자", 1077006812, 1);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("미금", 1077006813, 1);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("동천", 1077006814, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("수지구청", 1077006815, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("성복", 1077006816, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("상현", 1077006817, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("광교중앙(아주대)", 1077006818, 0);
		arCustomList.add(listnorm);
		listnorm = new ListNorm("광교(경기대)", 1077006819, 3);
		arCustomList.add(listnorm);

		adapter = new ListNormAdapter(this, tm.getListTheme(iTheme, LineManager.LINE_SHINBUNDANG, mPref.getInt("iLineStatus", 0)), arCustomList);
	...
	class ListNormAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			viewHolder.icon_station_down_norm.setVisibility(View.VISIBLE);
			viewHolder.icon_station_up_norm.setVisibility(View.VISIBLE);
			...
			if (arC.get(position).iStainfo == 0) {
				...
				viewHolder.icon_station_down_norm.setImageResource(R.drawable.icon_1077_norm);
				viewHolder.icon_station_up_norm.setImageResource(R.drawable.icon_1077_norm);
				...
				viewHolder.view_line3.setVisibility(View.VISIBLE);
				viewHolder.view_line4.setVisibility(View.VISIBLE);
				viewHolder.icon_station_down_norm.setImageResource(R.drawable.icon_1077_tran);
				viewHolder.icon_station_up_norm.setImageResource(R.drawable.icon_1077_tran);
				...
				viewHolder.view_line3.setVisibility(View.VISIBLE);
				viewHolder.view_line4.setVisibility(View.VISIBLE);
				viewHolder.icon_station_down_norm.setVisibility(View.INVISIBLE);
				viewHolder.icon_station_up_norm.setVisibility(View.INVISIBLE);
			}
	...
	public void SetFavor(int position, String station) {
		...
		AlertDialog.Builder alert = new AlertDialog.Builder(ShinBundangActivity.this, android.R.style.Theme_DeviceDefault_Light_Dialog);
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				switch(which) {
					...
					case 1:
						...
						lineManager.setFavorite(str_station, LineManager.LINE_SHINBUNDANG, index);
					...
					case 2:
						...
						dm.setStation("신분당선", str_station + ":" + index);
	...
	private String[][] jsonParserList(String pRecvServerPage) {
		...
		try {
			...
			for(int i=0; i<parseredData.length; i++){
				...
				if(station[i] == 1077006810 && status[i] == 1 && trainStatus[i].equals("출발")) {
					station[i] = 1077000000;
					trainStatus[i] = "주행";
				} else if(station[i] == 1077006811 && status[i] == 0 && trainStatus[i].equals("출발")){
					station[i] = 1077000000;
					trainStatus[i] = "주행";
				}
			}
